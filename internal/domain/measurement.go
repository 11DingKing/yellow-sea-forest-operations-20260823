...
func (m *ForestSurvey) Cancel(now time.Time) error {
	if m.Status != ForestSurveyDraft {
		return TransitionError{Entity: "forest_survey", From: string(m.Status), To: string(ForestSurveyCancelled)}
	}
	m.Status = ForestSurveyCancelled
	m.Version++
	m.UpdatedAt = now.UTC()
	return nil
}
