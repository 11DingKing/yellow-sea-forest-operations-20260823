...
func (j OutboxJob) OwnedBy(workerID ID, now time.Time) bool {
	return j.WorkerID != nil && *j.WorkerID == workerID && j.Status == JobRunning && j.LeaseExpiresAt != nil && j.LeaseExpiresAt.After(now.UTC())
}
