...
func (s *store) FailAction(ctx context.Context, id, workerID domain.ID, expectedVersion int64, reason string, now time.Time) error {
	result, err := s.queryer.ExecContext(ctx, `
		UPDATE forest_asset_actions SET status='failed',worker_id=?,lease_expires_at=lease_expires_at,last_error=?,version=version+1,updated_at=?
		WHERE id=? AND worker_id=? AND status='claimed' AND version=?`, workerID, reason, formatTime(now), id, workerID, expectedVersion)
	if err != nil {
		return mapError("fail forest_asset action", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return err
