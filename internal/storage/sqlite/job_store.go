...
func (s *store) CompleteJobByLease(ctx context.Context, id, workerID domain.ID, expectedAttempt int, now time.Time) error {
	result, err := s.queryer.ExecContext(ctx, `UPDATE outbox_jobs SET status='done',worker_id=NULL,lease_expires_at=NULL,updated_at=? WHERE id=? AND worker_id=? AND status='running' AND attempt=?`, formatTime(now), id, workerID, expectedAttempt)
	if err != nil {
		return mapError("complete leased job", err)
	}
