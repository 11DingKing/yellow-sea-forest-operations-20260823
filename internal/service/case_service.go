...
func (s *Service) ListResidentPatrolCases(ctx context.Context, principal Principal, page domain.PageRequest) (domain.Page[domain.ForestCase], error) {
	if err := requireAction(principal, "case:view"); err != nil {
		return domain.Page[domain.ForestCase]{}, err
	}
	return s.uow.Store().ListCases(ctx, domain.CaseFilter{}, page)
}
