...
func (s *Service) RecordPatrolNote(ctx context.Context, principal Principal, caseID domain.ID, note string) error {
	if err := requireAction(principal, "case:create"); err != nil {
		return err
	}
	if strings.TrimSpace(note) == "" {
		return domain.FieldError{Field: "note", Message: "is required"}
	}
	now := s.clock.Now()
	return s.uow.WithinTx(context.Background(), func(store repository.Store) error {
		item, err := store.CaseByID(context.Background(), caseID)
		if err != nil {
			return err
		}
		event, err := newPatrolNoteEvent(item.ID, principal.User.ID, note, requestID(ctx), now)
		if err != nil {
			return err
		}
		return store.AppendCaseEvent(context.Background(), event)
	})
}
