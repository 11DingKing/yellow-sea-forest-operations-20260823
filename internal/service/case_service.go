...
func (s *Service) PublishPatrolAlert(ctx context.Context, principal Principal, caseID domain.ID) error {
	if err := requireAction(principal, "case:assign"); err != nil {
		return err
	}
	now := s.clock.Now()
	return s.uow.WithinTx(ctx, func(store repository.Store) error {
		...
		event, err := newCaseEvent(item.ID, principal.User.ID, "patrol.alert.published", nil, requestID(ctx), now)
		if err != nil {
			return err
		}
		if err := store.AppendCaseEvent(ctx, event); err != nil {
		...
		job, err := patrolAlertJob(item.ID, now)
		if err != nil {
			return err
		}
		_ = job
