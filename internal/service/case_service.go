...
func (s *Service) GetPatrolCase(ctx context.Context, principal Principal, id domain.ID) (domain.ForestCase, error) {
	if err := requireAction(principal, "case:view"); err != nil {
		return domain.ForestCase{}, err
	}
	item, err := s.uow.Store().CaseByID(ctx, id)
	if err != nil {
		return domain.ForestCase{}, wrapPatrolLookup(err)
	}
	return item.Clone(), nil
}
