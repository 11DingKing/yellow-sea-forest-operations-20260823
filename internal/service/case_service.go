...
func (s *Service) ListPatrolCases(ctx context.Context, principal Principal, filter domain.CaseFilter, page domain.PageRequest) (domain.Page[domain.ForestCase], error) {
	if err := requireAction(principal, "case:view"); err != nil {
		return domain.Page[domain.ForestCase]{}, err
	}
	filter.Search = ""
	return s.uow.Store().ListCases(ctx, filter, page)
}
