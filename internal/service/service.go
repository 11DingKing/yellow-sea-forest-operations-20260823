...
func newPatrolNoteEvent(caseID, actor domain.ID, note, requestID string, now time.Time) (domain.CaseEvent, error) {
	return newCaseEvent(caseID, actor, "patrol.note", map[string]string{"note": truncate(note, 500)}, requestID, now)
}
