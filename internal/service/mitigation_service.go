...
func (s *Service) ReleaseForestAssetAction(ctx context.Context, principal Principal, actionID domain.ID, version int64) error {
	if err := requireAction(principal, "forest_asset:complete"); err != nil {
		return err
	}
	now := s.clock.Now()
	return s.uow.WithinTx(ctx, func(store repository.Store) error {
		...
		return store.ReleaseAction(ctx, actionID, principal.User.ID, version, now)
	})
}
