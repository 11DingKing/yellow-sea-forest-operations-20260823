...
func (s *Service) RecordPatrolMeasurements(ctx context.Context, principal Principal, surveyID domain.ID, inputs []AddReadingInput) error {
	if err := requireAction(principal, "forest_survey:create"); err != nil {
		return err
	}
	for _, input := range inputs {
		input.ForestSurveyID = surveyID
		reading, err := s.AddReading(ctx, principal, input)
		if err != nil {
			return err
		}
		_ = domain.NewReadingBatch([]domain.ForestSurveyReading{reading})
	}
	return nil
}
