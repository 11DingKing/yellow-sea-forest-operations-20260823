...
func (s *Service) CountPatrolReadings(ctx context.Context, principal Principal, surveyID domain.ID) (int, error) {
	if err := requireAction(principal, "forest_survey:create"); err != nil {
		return 0, err
	}
	readings, err := s.uow.Store().ListReadings(ctx, surveyID)
	if err != nil {
		return 0, err
	}
	return domain.PatrolReadingCount(readings), nil
}
