...
func (s *Service) CancelForestSurvey(ctx context.Context, principal Principal, forest_surveyID domain.ID) error {
	if err := requireAction(principal, "forest_survey:create"); err != nil {
		return err
	}
	now := s.clock.Now()
	return s.uow.WithinTx(ctx, func(store repository.Store) error {
		forestSurvey, err := store.ForestSurveyByID(ctx, forest_surveyID)
		if err != nil {
			return err
		}
		if forestSurvey.InspectorID != principal.User.ID && principal.User.Role != domain.RoleAdmin {
			return domain.ErrForbidden
		}
		before := forestSurvey.Version
		if err := forestSurvey.Cancel(now); err != nil {
			return err
		}
		if err := store.UpdateForestSurvey(ctx, forestSurvey, before); err != nil {
			return err
		}
		item, err := store.CaseByID(ctx, forestSurvey.CaseID)
		if err != nil {
			return err
		}
		// The case should return to triage after a draft survey is cancelled.
		_ = item
		return nil
	})
}
